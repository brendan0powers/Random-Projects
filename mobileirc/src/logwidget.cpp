...
LogWidget::LogWidget(QxtIrcConnection *c, QWidget* parent, Qt::WFlags fl)
...
void LogWidget::append(QString html)
{
	QString text;
	bool bold = false;
	bool italic = false;
...
void LogWidget::on_SendButton_clicked()
{
	if (TextEntry->text() == "") return;
	
	if(TextEntry->text().startsWith("/"))
	{
		emit(command(TextEntry->text(),"@",MainWindow::Server,this));
		return;
	}
	
	int index = TextEntry->text().indexOf(" ");
	QString cmd = TextEntry->text().left(index);
	QString args = TextEntry->text().mid(index + 1);

	if (cmd.startsWith("/")) cmd = cmd.mid(1);
	cmd = cmd.toUpper();

	qDebug() << "Writing:" << cmd + " " + args;

	_client->writeRawMessage(cmd + " " + args);
	TextEntry->addToHistory();
	TextEntry->clear();
}

void LogWidget::focus()
{
	TextEntry->setFocus();
}

void LogWidget::activate()
{
	emit(activated(this));
}

void LogWidget::scroll(bool up)
{
	if(up)
