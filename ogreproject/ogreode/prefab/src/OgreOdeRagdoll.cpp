...
void Ragdoll::update()
{
	...
	{		
            ...
            i != _root_bones.end();++i)
		{
			...
			{
				// if the root has no vertex assigned... it won't be in this list
				// therefore we must get its derived transform anyway.
				updatePhysicalBone(*i, 
					_node_trans * (*i)->_bone->getParent()->_getFullTransform(), 
					_node_trans);
			}
			else
			{
...
bool Ragdoll::pickTrimesh(RayGeometry *ray, Body* &body, Vector3 &position, World *world, Space* space)
{
	...
	OgreOde::TriangleMeshGeometry *trimeshGeom = _ei->createStaticTriangleMesh(_world, space);
