...
void Vehicle::Wheel::update(Real power_force,Real desired_rpm,Real brake_force)
{
	...
	const Real force = (power_force * _power_factor) - (brake_force * _brake_factor);
...
void Vehicle::Wheel::setSteer(Real position)
{
	...
	{
		const Real steer_angle = position * _steer_limit;
...
void Vehicle::update(Real time)
{
	...
	_engine->update(time);
	const Real power = _engine->getPowerAtRPM(rpm);
	const Real desired_rpm = _engine->getDesiredRPM();
	const Real brake = _engine->getBrakeForce();
