					...
					if (element->Attribute("mesh"))
					{
						...
						if (!vehicle->getGeometry()) 
						{
							...
							if (geometryName.empty()) 
								...
								vGeom  = new CapsuleGeometry(box.y*0.5, box.z*0.5, _world, _space);
							else if (geometryName == "capsuleZ")
								vGeom  = new CapsuleGeometry(box.y*0.5, box.z*0.5, _world, _space);
							else if (geometryName == "capsuleX")
								vGeom  = new CapsuleGeometry(box.y*0.5, box.x*0.5, _world, _space);
							else if (geometryName == "capsuleY")
								vGeom  = new CapsuleGeometry(box.x*0.5, box.y*0.5, _world, _space);
...
Ragdoll* DotLoader::parseRagdoll(const TiXmlNode *child, const String &ragdoll_name, const String &instance_name)
{
	...
	Ogre::String ragdoll_name_asked (ragdoll_name);
	Ogre::String ragdoll_name_from_file (ragdollElement->Attribute("name"));

	Ogre::StringUtil::toLowerCase(ragdoll_name_from_file);
	Ogre::StringUtil::toLowerCase(ragdoll_name_asked);
